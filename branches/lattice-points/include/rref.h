//
//code taken from http://rosettacode.org/wiki/Reduced_row_echelon_form#C.2B.2B
//
...
// specialization of the matrix traits for std::vector
template<typename T>
 struct matrix_traits<std::vector<std::vector<T> > >
{
  typedef std::size_t index_type;
  typedef T value_type;  
  static index_type min_row(std::vector<std::vector<T> > const& A)
  { return 0; }
  static index_type max_row(std::vector<std::vector<T> > const& A)
  { return A.size()-1; }
  static index_type min_column(std::vector<std::vector<T> > const& A)
  { return 0; }
  static index_type max_column(std::vector<std::vector<T> > const& A)
  { return (*(A.begin())).size()-1; }
  static value_type& element(std::vector<std::vector<T> > & A, index_type i, index_type k)
  { return A[i][k]; }
  static value_type element(std::vector<std::vector<T> > const& A, index_type i, index_type k)
  { return A[i][k]; }
};
