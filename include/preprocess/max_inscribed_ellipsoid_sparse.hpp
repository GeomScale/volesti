...
#ifndef MVE_COMPUTATION_SPARSE_HPP
#define MVE_COMPUTATION_SPARSE_HPP
...
std::pair<std::pair<MT, VT>, bool> max_inscribed_ellipsoid_sparse(MT _A, VT b, VT const& x0,
                                                   ...
                                                   NT const& tol, NT const& reg)
{
    ...
    A = _A.sparseView();   // FIXED: A is a sparse matrix
    ...
    DT Y(m);     // FIXED: Y is a diagonal matrix
    Eigen::SparseMatrix<NT> YA(m, n);   // FIXED: YA will be a sparse matrix

    A = (ones_m.cwiseProduct(bmAx0.cwiseInverse())).asDiagonal() * A, b = ones_m;   // FIXED: the operation will be faster for sparse A
    Eigen::SparseMatrix<NT> A_trans = A.transpose(); // FIXED: A_transpose will also be sparse for sparse A
