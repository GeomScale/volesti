...
#ifndef BALL_HVOL_VPOLY_H
#define BALL_HVOL_VPOLY_H

//#include "low_dimensional_sampling.h"
#include "ball_annealingGl.h"
#include "ball_hpoly_annealing.h"
...
NT ball_hvol_vpoly (Vpolytope &VP, UParameters &var, AParameters &var_ban, GParameters &var_g,
    ...
    BallPoly BP1(HP, B0);
    construct_hpoly(VP, HP, BP1, 10*n, k, var3);
    Hpolytope HP3;
    HP3.init(n,HP.get_mat(),HP.get_vec());
    HP3.normalize();
    BallPoly BP(HP3, B0);
    //HP3.print();
    get_first_ballpoly(VP, BP, lb, ub, ratio, var3);
    std::cout<<"radius of BP = "<<BP.radius()<<std::endl;
    NT vol22 = (std::pow(M_PI, n / 2.0) * (std::pow(BP.radius(), n))) / (tgamma(n / 2.0 + 1));
    std::cout<<", vol of BP_ball = "<<vol22<<std::endl;
    std::cout<<"number of hyperplanes = "<<BP.num_of_hyperplanes()<<std::endl;
    ...
    std::pair<Point, NT> InnerBall = BP.ComputeInnerBall();
    ...
    typedef ZonoIntersectHPoly<Vpolytope, BallPoly > ZonoHP;
    std::vector<BallPoly > HPolySet;
    ...
    get_sequence_of_vpoly_ballpolys<ZonoHP>(VP, BP, HPolySet, ratios, N*nu, nu, lb, ub, alpha, var, var3);
    var.diameter = diam0;
    ...
    vol = volume_gaussian_annealing(BP, var_g, var, InnerBall, fake_nballs);
    ...
    if (!window2) {
        ...
        vol *= esti_ratio_interval<RNGType, Point>(BP, VP, ratio, er0, win_len, N*nu, prob, var2);
    } else {
        vol *= esti_ratio<RNGType, Point>(BP, VP, ratio, er0, var_g.W, N*nu, var);
    }
    ...
    if (HPolySet.size()==0) {
        ...
        if (ratios[0]!=1) {
            if(!window2) {
                vol = vol / esti_ratio_interval<RNGType, Point>(VP, BP, ratios[0], er1, win_len, N*nu, prob, var);
            } else {
                vol = vol / esti_ratio<RNGType, Point>(VP, BP, ratios[0], er1, var_g.W, N*nu, var);
            }
        }
    } else {
        ...
        var.diameter = 2.0*zb1.radius();
        if (!window2) {
            vol = vol / esti_ratio_interval<RNGType, Point>(zb1, BP, ratios[ratios.size() - 1], er1, win_len, N*nu, prob, var);
        } else {
            vol = vol / esti_ratio<RNGType, Point>(zb1, BP, ratios[ratios.size() - 1], er1, var_g.W, N*nu, var);
        }
    }
