...
#ifndef SPARSE_HPOLYTOPE_H
#define SPARSE_HPOLYTOPE_H
...
public:
    ...
    SparseHPolytope(unsigned d_, MT const& A_, MT const& b_) :
        _d{d_}, A{A_}, b{b_}
    {
    }

    // Copy constructor
    SparseHPolytope(SparseHPolytope<Point> const& p) :
    ...
    SparseHPolytope(std::vector<std::vector<NT>> const& Pin)
    {
        _d = Pin[0][1] - 1;
        A = MT(Pin.size() - 1, _d);
        b = MT(Pin.size() - 1, 1);
    ...
    std::pair<Point, NT> ComputeInnerBall()
    {
        ...
        /*if (_inner_ball.second < 0.0) {
            ...
            std::tuple<MT, NT, bool> inner_ball = max_inscribed_ball(A, b, 150, tol);
    ...
    int is_in(Point const& p, NT tol=NT(0))
    {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (Eigen::SparseMatrix<NT>(b.row(i) - A.row(i) * p.getCoefficients()).coeff(0, 0) < NT(-tol)) {
                return 0;
            }
        }
        return -1;
    ...
    std::pair<NT,NT> line_intersect(Point &r, Point &v)
    {
        ...
        MT sum_nom = Eigen::SparseMatrix<NT>(b - A * r.getCoefficients());
        MT sum_denom = Eigen::SparseMatrix<NT>(A * v.getCoefficients());

        for (int i = 0; i < m; i++) {

            if (sum_denom.coeff(i, 0) == NT(0)) {
                //std::cout<<"div0"<<std::endl;
                ;
            } else {
                lamda = sum_nom.coeff(i, 0) / sum_denom.coeff(i, 0);
