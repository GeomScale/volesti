multiphase_sampling <- function(A, b, max_ball, n, num_rounding_samples, 
  ...
  while (!parameters$complete) {
    parameters = sample_step(max_ball$center, max_ball$radius, parameters)
    N = dim(parameters$correlated_samples)[2]
    neff_sampled = neff_sampled + parameters$Neff_sampled
    print(paste0("neff_sampled = ", neff_sampled))
