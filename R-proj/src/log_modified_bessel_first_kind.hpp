...
double log_sum_exp(const T2& a, const T1& b) {
  if (std::isinf(a)) {
    return b;
  }
 ...
 */
template <typename T1, typename T2>
 double log_modified_bessel_first_kind(
  ...
  if (v == 0) {
    ...
    if (z < 500) {
      ...
      return z + log(evaluate_polynomial(P, (T(1)/z))) - multiply_log(0.5, z);
    }
  ...
  if (z > 100) {
    ...
    //T lim = std::pow(std::pow(T(v), T(2)) + 2.5) / (2 * z), 3) / 24;
    T lim = std::pow((std::pow(T(v), T(2)) + 2.5) / (T(2) * z), T(3)) / T(24);
    if (lim < (EPSILON * 10)) {
      T s = 1;
      T mu = 4 * std::pow(T(v), T(2));
      T ex = 8 * z;
      T num = mu - 1;
      T denom = ex;
      s -= num / denom;
      ...
      denom *= ex * 3;
      s -= num / denom;
      s = z - std::log(sqrt(z * T(2)*M_PI)) + std::log(s);
      return s;
    }
  }

  T log_half_z = std::log(0.5 * z);
  T lgam = v > -1 ? std::lgamma(v + 1.0) : 0;
  T lcons = (2.0 + v) * log_half_z;
  T out;
  if (v > -1) {
    out = log_sum_exp(v * log_half_z - lgam, lcons - std::lgamma(v + 2));
    lgam += log1p(v);
  } else {
    out = lcons;
  }
