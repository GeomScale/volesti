...
#ifndef ANNEALING_ZONO_H
#define ANNEALING_ZONO_H
...
void get_annealing_schedule(Polytope &P, NT radius, NT ratio, NT C, NT frac, unsigned int N,
    ...
    const unsigned int steps= ((int)150/error)+1;
    std::list<Point> randPoints;
    ...
    while (true) {
        ...
        for (unsigned  int j = 0; j < steps; j++) {
            ...
            curr_its += 1.0;
            //curr_fn += eval_exp(p, next_a) / eval_exp(p, a_vals[it]);
            curr_fn += std::exp(-(next_a)*(pointset.col(j).squaredNorm())) / std::exp(-(a_vals[it])*(pointset.col(j).squaredNorm()));
        }
