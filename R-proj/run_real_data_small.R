...
n = dim(usa_covariance_matrices_small$lCov[[1]])[2] #number of assets
...
for (index in seq(from=start_index, to=end_index, by=5)) {
  ...
  for (i in index:(index+9)) {
    ...
    sigma = usa_covariance_matrices_small$lCov[[i]] #get the covariance matrix
  
    Cs = usa_covariance_matrices_small$lVola_targets[[i]] #get m level of volatilities
    m = length(Cs)
