...
source('~/volume_approximation/R-proj/R/metabolic_net_2_polytopes.R', echo=TRUE)

P = metabolic_net_2_polytope(path_mets, remove_biomass = FALSE, Recon2 = TRUE)
print("compute min and max Fluxes")
pre_proc_list = fast_preprocess_with_mosek(P)

save(pre_proc_list, file = "Recon2_preprocess_matrices.RData")


print("Compute the null space to constraint")
rr = full_dimensional_polytope_with_arma(pre_proc_list$Aeq, pre_proc_list$beq)

save(r, file = "Recon2_null_space.RData")
...
save(A, b, file = "Recon2_scaling.RData")
...
ret_list = rounding_isotropic(HP)
  ...
  save(ret_list, file = "Recon2_rounding_matrices.RData")
...
save(result_list, file = "Recon2_result_list.RData")
})

save(as.numeric(tim)[3], file = "time_Recon2.RData")
