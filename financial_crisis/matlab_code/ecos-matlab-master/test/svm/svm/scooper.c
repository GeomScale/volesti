...
#define NTFIELDS 8
  ...
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.X");
  } else {
    if (!( (mxGetM(xm) == 10) && (mxGetN(xm) == 2) )) mexErrMsgTxt("X must be size (10, 2)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter X must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter X must be full matrix of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter X must be full matrix");
    ptr = mxGetPr(xm);
    for(j = 0; j < 2; ++j) {
      for(i = 0; i < 10; ++i) {
        p.X[i][j] = *ptr++;
  ...
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.Y");
  } else {
    if (!( (mxGetM(xm) == 10) && (mxGetN(xm) == 2) )) mexErrMsgTxt("Y must be size (10, 2)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter Y must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter Y must be full matrix of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter Y must be full matrix");
    ptr = mxGetPr(xm);
    for(j = 0; j < 2; ++j) {
      for(i = 0; i < 10; ++i) {
        p.Y[i][j] = *ptr++;
  ...
  memcpy(mxGetPr(xm), v.a, sizeof(double)*2);
