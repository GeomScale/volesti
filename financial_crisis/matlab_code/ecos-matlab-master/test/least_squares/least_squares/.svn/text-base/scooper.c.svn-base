...
#define NTFIELDS 8
  ...
  if (xm == NULL) {
    ...
    if (!( (mxGetM(xm) == 10) && (mxGetN(xm) == 5) )) mexErrMsgTxt("A must be size (10, 5)\n");
  ...
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.C");
  } else {
    if (!( (mxGetM(xm) == 3) && (mxGetN(xm) == 5) )) mexErrMsgTxt("C must be size (3, 5)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter C must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter C must be full matrix of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter C must be full matrix");
    ptr = mxGetPr(xm);
    for(j = 0; j < 5; ++j) {
      for(i = 0; i < 3; ++i) {
        p.C[i][j] = *ptr++;
  ...
  if (xm == NULL) {
    ...
    if (!( (mxGetM(xm) == 10) && (mxGetN(xm) == 1) )) mexErrMsgTxt("b must be size (10, 1)\n");
    ...
    memcpy(p.b, mxGetPr(xm), sizeof(double)*10);
  }
  ...
  if (xm == NULL) {
    ...
    if (!( (mxGetM(xm) == 3) && (mxGetN(xm) == 1) )) mexErrMsgTxt("d must be size (3, 1)\n");
    ...
    memcpy(p.d, mxGetPr(xm), sizeof(double)*3);
  }
  ...
  memcpy(mxGetPr(xm), v.x, sizeof(double)*5);
