...
#define NTFIELDS 8
  ...
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.F");
  } else {
    if (!( (mxGetM(xm) == 10) && (mxGetN(xm) == 3) )) mexErrMsgTxt("F must be size (10, 3)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter F must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter F must be full matrix of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter F must be full matrix");
    ptr = mxGetPr(xm);
    for(j = 0; j < 3; ++j) {
      for(i = 0; i < 10; ++i) {
        p.F[i][j] = *ptr++;
  ...
  if (xm == NULL) {
    ...
    if (!( (mxGetM(xm) == 10) && (mxGetN(xm) == 1) )) mexErrMsgTxt("d must be size (10, 1)\n");
    ...
    memcpy(p.d, mxGetPr(xm), sizeof(double)*10);
  }

  xm = mxGetField(prhs[0], 0, "gamma");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.gamma");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("gamma must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter gamma must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter gamma must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter gamma must be full vector");
    p.gamma = *mxGetPr(xm);
  }

  xm = mxGetField(prhs[0], 0, "mu");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.mu");
  } else {
    if (!( (mxGetM(xm) == 10) && (mxGetN(xm) == 1) )) mexErrMsgTxt("mu must be size (10, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter mu must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter mu must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter mu must be full vector");
    memcpy(p.mu, mxGetPr(xm), sizeof(double)*10);
  }
  ...
  memcpy(mxGetPr(xm), v.x, sizeof(double)*10);
