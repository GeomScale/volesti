...
#define NTFIELDS 8
  ...
  if (xm == NULL) {
    ...
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("a must be size (1, 1)\n");
  ...
  xm = mxGetField(prhs[0], 0, "one_over_q");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.one_over_q");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("one_over_q must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter one_over_q must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter one_over_q must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter one_over_q must be full vector");
    p.one_over_q = *mxGetPr(xm);
  }

  xm = mxGetField(prhs[0], 0, "p");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.p");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("p must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter p must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter p must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter p must be full vector");
    p.p = *mxGetPr(xm);
  }

  xm = mxGetField(prhs[0], 0, "q");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.q");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("q must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter q must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter q must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter q must be full vector");
    p.q = *mxGetPr(xm);
  }

  xm = mxGetField(prhs[0], 0, "r");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.r");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("r must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter r must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter r must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter r must be full vector");
    p.r = *mxGetPr(xm);
  }

  xm = mxGetField(prhs[0], 0, "x_0");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.x_0");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("x_0 must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter x_0 must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter x_0 must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter x_0 must be full vector");
    p.x_0 = *mxGetPr(xm);
  }
  ...
  int flag = 0;
  flag = solve(mywork, &v);
  const int num_var_names = 7;
  const char *var_names[] = {"x1", "x2", "x3", "x4", "x5", "x6", "x7"};
  ...
  *mxGetPr(xm) = v.x2;
  ...
  xm = mxCreateDoubleMatrix(1, 1,mxREAL);
  mxSetField(plhs[0], 0, "x3", xm);
  *mxGetPr(xm) = v.x3;

  xm = mxCreateDoubleMatrix(1, 1,mxREAL);
  mxSetField(plhs[0], 0, "x4", xm);
  *mxGetPr(xm) = v.x4;

  xm = mxCreateDoubleMatrix(1, 1,mxREAL);
  mxSetField(plhs[0], 0, "x5", xm);
  *mxGetPr(xm) = v.x5;

  xm = mxCreateDoubleMatrix(1, 1,mxREAL);
  mxSetField(plhs[0], 0, "x6", xm);
  *mxGetPr(xm) = v.x6;

  xm = mxCreateDoubleMatrix(1, 1,mxREAL);
  mxSetField(plhs[0], 0, "x7", xm);
  *mxGetPr(xm) = v.x7;
