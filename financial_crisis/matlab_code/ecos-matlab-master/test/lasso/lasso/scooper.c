...
#define NTFIELDS 8
  ...
  if (xm == NULL) {
    ...
    if (!( (mxGetM(xm) == 2) && (mxGetN(xm) == 3) )) mexErrMsgTxt("A must be size (2, 3)\n");
  ...
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.l");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("l must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter l must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter l must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter l must be full vector");
    p.l = *mxGetPr(xm);
  }

  xm = mxGetField(prhs[0], 0, "lambda");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.lambda");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("lambda must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter lambda must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter lambda must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter lambda must be full vector");
    p.lambda = *mxGetPr(xm);
  }

  xm = mxGetField(prhs[0], 0, "u");
  if (xm == NULL) {
    mexErrMsgTxt("could not find params.u");
  } else {
    if (!( (mxGetM(xm) == 1) && (mxGetN(xm) == 1) )) mexErrMsgTxt("u must be size (1, 1)\n");
    if (mxIsComplex(xm)) mxErrMsgTxt("parameter u must be real\n");
    if (!mxIsClass(xm, "double")) mxErrMsgTxt("parameter u must be full vector of doubles");
    if (mxIsSparse(xm)) mxErrMsgTxt("parameter u must be full vector");
    p.u = *mxGetPr(xm);
  }
  ...
  memcpy(mxGetPr(xm), v.x, sizeof(double)*3);
