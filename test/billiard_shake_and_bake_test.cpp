...
MT get_samples_billiard_shake_and_bake(Polytope &P)
{
    ...
    billiard_shakeandbake_sampling<WalkType>(randPoints,P,rng,walkL,nr,numpoints,boundary_pt, nburns,facet_idx);        
...
template <typename NT, typename WalkType = BilliardShakeAndBakeWalk>
void call_test_billiard_shake_and_bake(){
    ...
    MT samples = get_samples_billiard_shake_and_bake<MT, WalkType>(P);
...
TEST_CASE("billiard_shake_and_bake") {
    call_test_billiard_shake_and_bake<double>();
}