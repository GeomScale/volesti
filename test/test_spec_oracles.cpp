...
void call_test_intersection() {
    ...
    std::pair<NT, NT> ret = spectrahedron.intersection(initialPoint, objFunction.getCoefficients(), precomputedValues);
    ...
    if (std::fabs(ret.first - 0.809786) > 0.0001 || std::fabs(ret.second + 0.729182) > 0.0001)
        correct = false;
...
void call_test_positive_intersection() {
    ...
    SPECTRAHEDRON::PrecomputedValues precomputedValues;
    NT ret = spectrahedron.positiveIntersection(initialPoint, objFunction.getCoefficients(), precomputedValues);
    ...
    if (std::fabs(ret - 0.809786) > 0.0001)
        correct = false;

    if (correct) {
        // test reflection
        VT intersectionPoint = initialPoint + ret * objFunction.getCoefficients();
        VT reflectedTrajectory;

        // compute reflection
        spectrahedron.computeReflection(intersectionPoint, objFunction.getCoefficients(), reflectedTrajectory, precomputedValues);

        // exact is
        // -1.37715, -0.321654
        if (std::fabs(reflectedTrajectory(0) + 1.37715) > 0.0001 || std::fabs(reflectedTrajectory(1) + 0.321654) > 0.0001)
            correct = false;
    }
...
TEST_CASE ("test_spec_oracles") {
    call_test_intersection(); // hit and run boundary oracle!
    call_test_positive_intersection(); // billiard boundary oracle! also test reflection
