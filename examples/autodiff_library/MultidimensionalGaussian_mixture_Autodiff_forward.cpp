...
struct autoDiffFunctor
{
    ...
    static autodiff::detail::Variable<NT> userDefinedFunction(const Eigen::Matrix<autodiff::detail::Variable<NT>, Eigen::Dynamic, 1> &x,  Eigen::Matrix<NT, Eigen::Dynamic, Eigen::Dynamic>& data)
    { 
        int d=x.rows();
    auto firstX=x.head(d/2);
    auto secondX=x.tail(d/2);// d*1 data_ n*d

    autodiff::detail::Variable< NT> SUM=0;
