...
struct autoDiffFunctor
{
    ...
    {
    int d=x.rows();
    auto firstX=x.head(d/2);
    auto secondX=x.tail(d/2);// d*1 data_ n*d

    autodiff::detail::Real<1, NT> SUM=0;
    auto I=Eigen::Matrix<NT,Eigen::Dynamic,Eigen::Dynamic>::Identity(d,d);
    for(int i=0;i<data_.rows();i++)
    {
        auto x_mu_first= data_.row(i).transpose() - firstX; // d*1
        auto x_mu_second= data_.row(i).transpose() -secondX; // d*1
...
void run_main()
{
    ...
    unsigned int dim = 40; // two times the dimension of  the data sampled
   ...
   int n_samples = 50000; // Half will be burned // 500 cannot 2000 cannot 
