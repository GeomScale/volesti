// Use backward-mode automatic differentiation using Autodiff Library
...
struct autoDiffFunctor
{
    ...
    static autodiff::detail::Variable<NT> userDefinedFunction(const Eigen::Matrix<autodiff::detail::Variable<NT>, Eigen::Dynamic, 1> &x)
    {
        auto temp = x.array();
    ...
    struct GradientFunctor
    {
        ...
        Point operator()(unsigned int const &i, pts const &xs, NT const &t) const
        {
            if (i == params.order - 1)
            {
                ...
                Eigen::Matrix<NT, Eigen::Dynamic, 1> result = autodiff::gradient(autoDiffFunctor::userDefinedFunction<NT>(temp), temp);
                Point y(result * -1);
                return y;
            }
            else
